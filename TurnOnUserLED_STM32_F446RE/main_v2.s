...
LED_ON					EQU 0 ; (1 << 5) ; 0b0000 0000 0000 0000 0000 0000 0010 0000
	...
	ENTRY ; entry point, optionaly is here
	EXPORT  Reset_Handler ; entry point search by linker configuration
...
Infinite_loop
	BL	Turn_On_LED
	B	Infinite_loop ; branch to same label
...
System_Init
	...
	ALIGN ; remove padding
		
Turn_On_LED
	LDR R0, =GPIOA_ODR ; loads the value of address GPIOA_ODR into R0
	LDR	R1, [R0] ; loads R1 from the address in R0. Get contents found at address R0 in R1
	ORR R1, R1, #LED_ON ; R1 = R1 | LED_ON
	STR	R1, [R0] ; contents of R1 stored to GPIOA_ODR
	BX LR ; end sub routine
	ALIGN ; remove padding
